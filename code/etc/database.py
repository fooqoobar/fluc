...
class Database:
    ...
    def base_connect(self) -> None:
        ...
        self.config = self.get_config(as_dict=True)
    ...
    def check(self) -> None:
        ...
        self.execute('CREATE TABLE IF NOT EXISTS user_blacklist(id INTEGER PRIMARY KEY)')
        self.execute('CREATE TABLE IF NOT EXISTS server_blacklist(id INTEGER PRIMARY KEY)')
    ...
    def sync(self) -> None:
        ...
        cells = self.execute('SELECT id FROM user_blacklist', fetch_all=True)
        for user_id in to_list(cells):
            user_id = user_id[0]
            if not user_id in self.blacklisted_users:
                self.blacklisted_users.append(user_id)

        cells = self.execute('SELECT id FROM server_blacklist', fetch_all=True)
        for server_id in to_list(cells):
...
class User:
    ...
    def blacklisted(self):
        return self.is_blacklisted if not self.elevated else False

    @property
    def premium(self):
        return self.is_premium or self.elevated
